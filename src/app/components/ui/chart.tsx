...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-[8rem] items-start gap-1.5 rounded-xl border border-white/10 bg-black/80 backdrop-blur-md px-3 py-2 text-xs shadow-xl",
        className,
      )}
    >
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                "flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 text-white/70",
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 text-white/60",
            )}
          >
